...
void HippoChargeTransferForceImpl::initialize(ContextImpl& context) {
   kernel = context.getPlatform().createKernel(
      CalcHippoChargeTransferForceKernel::Name(), context);
   kernel.getAs<CalcHippoChargeTransferForceKernel>().initialize(
      context.getSystem(), owner);
}

double HippoChargeTransferForceImpl::calcForcesAndEnergy(
   ContextImpl& context, bool includeForces, bool includeEnergy, int groups) {
   if ((groups & (1 << owner.getForceGroup())) != 0)
      return kernel.getAs<CalcHippoChargeTransferForceKernel>().execute(
