   ...
   */
   for (int atom = blockIdx.x * blockDim.x + threadIdx.x; atom < NUM_ATOMS;
      ...
      if (atomAxisInfo.x >= 0 && atomAxisInfo.z >= 0) {
         ...
         real3 vectorZ         = make_real3(posZ.x - thisParticlePos.x,
            posZ.y - thisParticlePos.y, posZ.z - thisParticlePos.z);
         real4 posX            = posq[atomAxisInfo.x];
         real3 vectorX         = make_real3(posX.x - thisParticlePos.x,
            posX.y - thisParticlePos.y, posX.z - thisParticlePos.z);
         ...
         if (axisType == 1) {
            ...
            if (atomAxisInfo.y >= 0 && atomAxisInfo.y < NUM_ATOMS) {
               ...
               real3 vectorY = make_real3(posY.x - thisParticlePos.x,
                  posY.y - thisParticlePos.y, posY.z - thisParticlePos.z);
         ...
         bool reverse = false;
         if (axisType != 0 && atomAxisInfo.x >= 0 && atomAxisInfo.y >= 0
            && atomAxisInfo.z >= 0) {
            real4 posY = posq[atomAxisInfo.y];
            real  delta[4][3];
            ...
            real volume = delta[3][0] * delta[0][0] + delta[3][1] * delta[1][0]
               + delta[3][2] * delta[2][0];
            reverse = (volume < 0);
         }
         ...
         if (reverse)
            molDipole[1] *= -1;
         globalFrameDipoles[offset] = molDipole[0] * vectorX.x
            + molDipole[1] * vectorY.x + molDipole[2] * vectorZ.x;
         globalFrameDipoles[offset + 1] = molDipole[0] * vectorX.y
            + molDipole[1] * vectorY.y + molDipole[2] * vectorZ.y;
         globalFrameDipoles[offset + 2] = molDipole[0] * vectorX.z
            + molDipole[1] * vectorY.z + molDipole[2] * vectorZ.z;
         ...
         globalFrameQuadrupoles[offset] = vectorX.x
               * (vectorX.x * mPoleXX + vectorY.x * mPoleXY
                    + vectorZ.x * mPoleXZ)
            + vectorY.x
               * (vectorX.x * mPoleXY + vectorY.x * mPoleYY
                    + vectorZ.x * mPoleYZ)
            + vectorZ.x
               * (vectorX.x * mPoleXZ + vectorY.x * mPoleYZ
                    + vectorZ.x * mPoleZZ);
         globalFrameQuadrupoles[offset + 1] = vectorX.x
               * (vectorX.y * mPoleXX + vectorY.y * mPoleXY
                    + vectorZ.y * mPoleXZ)
            + vectorY.x
               * (vectorX.y * mPoleXY + vectorY.y * mPoleYY
                    + vectorZ.y * mPoleYZ)
            + vectorZ.x
               * (vectorX.y * mPoleXZ + vectorY.y * mPoleYZ
                    + vectorZ.y * mPoleZZ);
         globalFrameQuadrupoles[offset + 2] = vectorX.x
               * (vectorX.z * mPoleXX + vectorY.z * mPoleXY
                    + vectorZ.z * mPoleXZ)
            + vectorY.x
               * (vectorX.z * mPoleXY + vectorY.z * mPoleYY
                    + vectorZ.z * mPoleYZ)
            + vectorZ.x
               * (vectorX.z * mPoleXZ + vectorY.z * mPoleYZ
                    + vectorZ.z * mPoleZZ);
         globalFrameQuadrupoles[offset + 3] = vectorX.y
               * (vectorX.y * mPoleXX + vectorY.y * mPoleXY
                    + vectorZ.y * mPoleXZ)
            + vectorY.y
               * (vectorX.y * mPoleXY + vectorY.y * mPoleYY
            ...
            + vectorZ.y
               * (vectorX.y * mPoleXZ + vectorY.y * mPoleYZ
                    + vectorZ.y * mPoleZZ);
         globalFrameQuadrupoles[offset + 4] = vectorX.y
               * (vectorX.z * mPoleXX + vectorY.z * mPoleXY
                    + vectorZ.z * mPoleXZ)
            + vectorY.y
               * (vectorX.z * mPoleXY + vectorY.z * mPoleYY
         ...
         globalFrameQuadrupoles[5 * atom + 1]
            = localFrameQuadrupoles[5 * atom + 1];
         globalFrameQuadrupoles[5 * atom + 2]
            = localFrameQuadrupoles[5 * atom + 2];
         globalFrameQuadrupoles[5 * atom + 3]
            = localFrameQuadrupoles[5 * atom + 3];
         globalFrameQuadrupoles[5 * atom + 4]
            = localFrameQuadrupoles[5 * atom + 4];
      }
   }
}
